"""Fine-tune CodeGen on MBPP for Python code generation."""
...
def ensure_mbpp_dataset() -> None:
    processed_train = PROCESSED_DIR / "mbpp_train.jsonl"
    if not processed_train.exists():
        logger.info("MBPP processed data missing; downloading...")
        download_mbpp()


def load_mbpp_datasets(
    ...
    for split in ["train", "validation"]:
        path = PROCESSED_DIR / f"mbpp_{split}.jsonl"
        if path.exists():
...
def train(args: argparse.Namespace) -> None:
    datasets = load_mbpp_datasets(args.max_train_samples, args.max_eval_samples, full=args.full)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fine-tune CodeGen on MBPP.")
